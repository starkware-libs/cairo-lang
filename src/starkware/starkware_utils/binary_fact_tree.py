...
class BinaryFactTree(ValidatedMarshmallowDataclass):
    ...
    async def empty_tree(
            cls, ffc: FactFetchingContext, height: int, leaf_fact: Fact) -> 'BinaryFactTree':
        """
    ...
    async def get_leaves(
            self, ffc: FactFetchingContext, indices: Collection[int], fact_cls: Type[TFact],
            facts: Optional[BinaryFactDict] = None) -> Dict[int, TFact]:
        """
    ...
    @abstractmethod
    async def update(
            self, ffc: FactFetchingContext, modifications: Collection[Tuple[int, Fact]],
            facts: Optional[BinaryFactDict] = None) -> 'BinaryFactTree':
        """
