...
from typing import (
    Any, AsyncIterator, Collection, Dict, NamedTuple, Optional, Tuple, Type, TypeVar, Union, cast)

from starkware.starkware_utils.binary_fact_tree import BinaryFactDict, TFact
from starkware.starkware_utils.merkle_tree.traverse_tree import traverse_tree
...
NodeType = NamedTuple('NodeType', [
    ('index', int), ('tree', 'BinaryFactTreeNode'), ('update', UpdateTree)])
...
class BinaryFactTreeNode(ABC):
    ...
    @abstractmethod
    async def combine(
            cls: Type[TBinaryFactTreeNode], ffc: FactFetchingContext, left: 'BinaryFactTreeNode',
            right: 'BinaryFactTreeNode',
            facts: Optional[BinaryFactDict] = None) -> TBinaryFactTreeNode:
        """
    ...
    async def get_children(
            self, ffc: FactFetchingContext, facts: Optional[BinaryFactDict] = None) -> Tuple[
                'BinaryFactTreeNode', 'BinaryFactTreeNode']:
        """
    ...
    async def _get_leaves(
            self, ffc: FactFetchingContext, indices: Collection[int],
            fact_cls: Type[TFact], facts: Optional[BinaryFactDict] = None) -> Dict[int, TFact]:
        """
        ...
        def unify_leaves(
                left_leaves: Dict[int, TFact], right_leaves: Dict[int, TFact]) -> Dict[int, TFact]:
    ...
    async def _update(
            self: TBinaryFactTreeNode, ffc: FactFetchingContext,
...
async def get_node_fact_or_fail(
        ffc: FactFetchingContext, node_fact_cls: Type[TFact], fact_hash: bytes) -> TFact:
...
async def read_node_fact(
        ffc: FactFetchingContext, inner_node_fact_cls: Type[TInnerNodeFact], fact_hash: bytes,
        facts: Optional[BinaryFactDict]) -> TInnerNodeFact:
...
async def write_node_fact(
        ffc: FactFetchingContext, inner_node_fact: InnerNodeFact,
        facts: Optional[BinaryFactDict]) -> bytes:
