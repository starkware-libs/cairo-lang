...
# Initializes the dict manager. Should be called exactly once at the beginning of a program that
# uses dicts.
func initialize_dict_manager() -> ():
    %{
        from starkware.cairo.common.dict import DictManager
        assert 'dict_manager' not in globals(), \
            'initialize_dict_manager() must be called exactly once.'
...
# Creates a new dict.
# Note that a dict_manager must be passed in the hints.
# Allocate one using initialize_dict_manager().
func dict_new() -> (res):
    %{ memory[ap] = dict_manager.new_dict(segments, initial_dict) %}
    ap += 1
    return (...)
end
...
# It is possible to get prev_value from dict_manager using the hint:
#   %{ ids.val = dict_manager.get_dict(ids.dict_ptr)[ids.key] %}
