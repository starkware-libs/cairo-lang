        ...
        for member_info in members_list:
            ...
            if name in members:
                raise PreprocessorError(
                    f"Redefinition of '{struct_name + name}'.", location=member_info.location
                )
    ...
    def create_struct_from_identifier_list(
        ...
        if identifier_list is not None:
            for arg in identifier_list.identifiers:
                ...
                members_list.append(
                    MemberInfo(
                        name=arg.identifier.name, cairo_type=arg.get_type(), location=arg.location
                    )
                )
    ...
    def visit_CodeElementFunction(self, elm: CodeElementFunction):
        ...
        if elm.element_type == "struct":
            if len(elm.decorators) != 0:
                raise PreprocessorError(
                    "Decorators for structs are not supported.", location=elm.decorators[0].location
                )
