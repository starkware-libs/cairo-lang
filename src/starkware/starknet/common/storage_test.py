...
def test_storage_read(runner: CairoFunctionRunner, structs: CairoStructProxy):
    ...
    assert runner.vm_memory.get_range(storage_ptr, storage_end - storage_ptr) == list(
        structs.DictAccess(key=address, prev_value=value, new_value=value)
    )
...
def test_storage_write(runner: CairoFunctionRunner, structs: CairoStructProxy):
    ...
    assert runner.vm_memory.get_range(storage_ptr, storage_end - storage_ptr) == list(
        structs.DictAccess(key=address, prev_value=orig_value, new_value=new_value)
    )
