...
func g{storage_ptr : Storage*, pedersen_ptr : HashBuiltin*, range_check_ptr}():
    alloc_locals
    let (x) = my_var.read()
...
[ap] = [ap + (-2)]; ap++       # Return (updated) storage_ptr.
...
@external
func test{storage_ptr : Storage*, range_check_ptr, pedersen_ptr : HashBuiltin*}() -> (x : felt):
    return f.read()
end
""",
        """
...
autogen/starknet/storage_var/f/decl.cairofile:?:?
    func read{storage_ptr : Storage*, pedersen_ptr : HashBuiltin*, range_check_ptr}() -> (x : felt):
              ^******************************************************************^
