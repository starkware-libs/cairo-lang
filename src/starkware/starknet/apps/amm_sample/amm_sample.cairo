...
const ACCOUNT_BALANCE_BOUND = 1073741
...
func get_account_token_balance{syscall_ptr : felt*, pedersen_ptr : HashBuiltin*, range_check_ptr}(
    ...
    let (res) = account_balance.read(account_id, token_type)
    return (balance=res)
end
...
func get_opposite_token(token_type : felt) -> (token : felt):
    if token_type == TOKEN_TYPE_A:
        return (token=TOKEN_TYPE_B)
    else:
        return (token=TOKEN_TYPE_A)
    end
end
