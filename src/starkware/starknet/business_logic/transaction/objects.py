...
from starkware.starknet.business_logic.utils import (
    ...
    validate_entrypoint_execution_context,
    verify_version,
...
class InternalAccountTransaction(InternalTransaction):
    ...
    def verify_version(self):
        verify_version(version=self.version, only_query=False)
    ...
    def run_validate_entrypoint(
        ...
        with validate_entrypoint_execution_context(resources_manager=resources_manager):
...
class InternalDeclare(InternalAccountTransaction):
    ...
    def _apply_specific_state_updates(
        ...
        actual_resources = calculate_tx_resources(
            resources_manager=resources_manager, call_infos=[validate_info], tx_type=self.tx_type
        )
        fee_transfer_info, actual_fee = self.charge_fee(
            state=state, general_config=general_config, resources=actual_resources
        )
...
class InternalDeploy(InternalTransaction):
    ...
    def handle_empty_constructor(self) -> TransactionExecutionInfo:
        stark_assert(
        ...
        resources_manager = ExecutionResourcesManager.empty()
        actual_resources = calculate_tx_resources(
            resources_manager=resources_manager, call_infos=[call_info], tx_type=self.tx_type
        )
...
class InternalInvokeFunction(InternalAccountTransaction):
    ...
    def _get_selector_from_external_tx(cls, tx: InvokeFunction) -> int:
        if tx.version in [0, constants.QUERY_VERSION_BASE]:
            assert tx.entry_point_selector is not None
            return tx.entry_point_selector

        assert tx.entry_point_selector is None
