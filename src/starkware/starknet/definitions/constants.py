...
class GasCost(Enum):
    ...
    # Syscall cas costs.
    CALL_CONTRACT = 18 * RANGE_CHECK + 903 * STEP
    DEPLOY = 7 * PEDERSEN + 21 * RANGE_CHECK + 1173 * STEP
    DEPLOY_CALLDATA_FACTOR = 1 * PEDERSEN + 8 * STEP
    GET_BLOCK_HASH = 2 * RANGE_CHECK + 107 * STEP
    GET_CLASS_HASH_AT = 1 * SYSCALL_BASE
    GET_EXECUTION_INFO = 2 * RANGE_CHECK + 125 * STEP

    # Secp256k1.
    SECP256K1_ADD = 29 * RANGE_CHECK + 412 * STEP
    SECP256K1_GET_POINT_FROM_X = 30 * RANGE_CHECK + 397 * STEP
    SECP256K1_GET_XY = 11 * RANGE_CHECK + 209 * STEP
    SECP256K1_MUL = 7045 * RANGE_CHECK + 76507 * STEP
    SECP256K1_NEW = 35 * RANGE_CHECK + 463 * STEP

    # Secp256r1.
    SECP256R1_ADD = 57 * RANGE_CHECK + 595 * STEP
    SECP256R1_GET_POINT_FROM_X = 44 * RANGE_CHECK + 516 * STEP
    SECP256R1_GET_XY = 11 * RANGE_CHECK + 211 * STEP
    SECP256R1_MUL = 13961 * RANGE_CHECK + 125346 * STEP
    SECP256R1_NEW = 49 * RANGE_CHECK + 582 * STEP

    KECCAK = 1 * SYSCALL_BASE
    KECCAK_ROUND_COST = 6 * BITWISE_BUILTIN + 1 * KECCAK_BUILTIN + 56 * RANGE_CHECK + 281 * STEP
    SHA256_PROCESS_BLOCK = 1115 * BITWISE_BUILTIN + 65 * RANGE_CHECK + 1867 * STEP
    LIBRARY_CALL = 18 * RANGE_CHECK + 879 * STEP
    REPLACE_CLASS = 1 * RANGE_CHECK + 106 * STEP
    STORAGE_READ = 1 * SYSCALL_BASE
    STORAGE_WRITE = 1 * SYSCALL_BASE
    EMIT_EVENT = 1 * SYSCALL_BASE
    SEND_MESSAGE_TO_L1 = 1 * RANGE_CHECK + 144 * STEP

    META_TX_V0 = 9 * PEDERSEN + 20 * RANGE_CHECK + 1301 * STEP
    META_TX_V0_CALLDATA_FACTOR = 1 * PEDERSEN + 8 * STEP
