...
def deploy_contract(batch_prover: BatchProver, w3: Web3, operator: eth.Account) -> eth.Contract:
    ...
    artifacts = subprocess.check_output(
        ['solc', '--bin', '--abi', CONTRACT_SOURCE_PATH]).decode('utf-8').split('\n')
    bytecode = artifacts[3]
    abi = artifacts[5]
    new_contract = w3.eth.contract(abi=abi, bytecode=bytecode)
    transaction = new_contract.constructor(
        accountTreeRoot=account_tree_root, amountTokenA=amount_token_a, amountTokenB=amount_token_b,
        cairoProgramHash=program_hash, cairoVerifier=cairo_verifier)
...
def main():
    ...
    operator_private_key_str = input(
        'Please enter an operator private key, '
        'or press Enter to generate a new private key: ')
    try:
